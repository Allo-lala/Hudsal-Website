...
export function StaffRequestModal({ isOpen, onClose }: StaffRequestModalProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    // Special handling for Virtual Companionship selection
    if (name === "serviceType" && value === "Virtual Companionship") {
      ...
      if (!confirmed) {
        // User cancelled, don't update the service type
        return;
      }
    }
    ...
    <>
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-2xl shadow-2xl animate-scale-in max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {/* Service Selection */}
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-foreground">Service Required</h3>
            ...
            {formData.serviceType === "Other" && (
              <div>
                <label htmlFor="otherService" className="block text-sm font-medium text-foreground mb-2">
                  Please describe the service you need <span className="text-red-500">*</span>
                </label>
                <textarea
                  id="otherService"
                  name="otherService"
                  ...
                  rows={4}
                  className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald ${
                    errors.otherService ? "border-red-500" : "border-border"
                  }`}
                  placeholder="Briefly describe the staff or service you're requesting..."
                />
          ...
          {submitStatus === "success" && (
            <div className="bg-emerald/10 border border-emerald text-emerald px-4 py-3 rounded-lg">
              Thank you! Your staff request has been submitted successfully. We'll be in touch soon.
            </div>
          )}
