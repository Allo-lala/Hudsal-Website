...
export async function POST(request: Request) {
  try {
    ...
    if (!firstName || !lastName || !email || !phone || !businessName || !businessEmail || !address || !serviceType) {
    ...
    const { data, error } = await resend.emails.send({
      from: "Hadsul Staff Request <noreply@hadsul.co.uk>",
      to: process.env.CONTACT_EMAIL || "info@hadsul.co.uk",
      replyTo: email,
      subject: `Staff Request: ${serviceType} - ${businessName}`,
      html: `
        <!DOCTYPE html>
        <html>
          <head>
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
              <h1 style="color: white; margin: 0; font-size: 28px;">Staff Request</h1>
            </div>
            ...
            <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
              ...
              <h2 style="color: #10b981;">Service Request</h2>
              <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                <tr>
              ...
              <div style="background: white; padding: 20px; border-radius: 8px; margin-top: 20px; border-left: 4px solid #10b981;">
                <p style="margin: 0; color: #6b7280; font-size: 14px;">
                  This staff request was submitted through the Hadsul website. Please respond promptly to provide excellent customer service.
                </p>
              </div>
            </div>
