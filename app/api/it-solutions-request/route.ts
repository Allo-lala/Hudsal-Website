...
export async function POST(request: Request) {
  try {
    ...
    if (!firstName || !lastName || !email || !phone || !businessName || !businessEmail || !address || !serviceType || !projectDescription) {
    ...
    const { data, error } = await resend.emails.send({
      from: "Hadsul IT Solutions <noreply@hadsul.co.uk>",
      to: process.env.CONTACT_EMAIL || "info@hadsul.co.uk",
      replyTo: email,
      subject: `IT Solutions Request: ${serviceType} - ${businessName}`,
      html: `
        <!DOCTYPE html>
        <html>
          <head>
            ...
            <title>IT Solutions Request</title>
          </head>
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
              <h1 style="color: white; margin: 0; font-size: 28px;">IT Solutions Request</h1>
            </div>
            ...
            <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
              ...
              <h2 style="color: #10b981;">Project Details</h2>
              <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                <tr>
                  <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><strong>Service Type:</strong></td>
                  <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><span style="background: #6366f1; color: white; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: bold;">${serviceType}</span></td>
                </tr>
              </table>

              <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #6366f1;">
                <h3 style="margin-top: 0; color: #6366f1;">Project Description</h3>
                <p style="margin: 0; color: #374151; white-space: pre-wrap;">${projectDescription}</p>
              </div>

              <div style="background: white; padding: 20px; border-radius: 8px; margin-top: 20px; border-left: 4px solid #10b981;">
                <p style="margin: 0; color: #6b7280; font-size: 14px;">
                  This IT solutions request was submitted through the Hadsul website. Please respond promptly to discuss the project requirements and provide a tailored solution.
                </p>
              </div>
            </div>
    ...
    console.error("IT solutions request error:", error);
    return NextResponse.json(
